...
for arg in "$@"; do
    case $arg in
        ...
        *)
            ...
            echo ""
            echo -e "${BLUE}=== Research Dashboard 部署脚本使用说明 ===${NC}"
            echo ""
            echo -e "${YELLOW}基本用法:${NC}"
            echo "  ./deploy.sh                    # 自动检测修改并部署（推荐）"
            echo "  ./deploy.sh --frontend         # 强制构建前端"
            echo "  ./deploy.sh --backend          # 仅推送后端代码"
            echo "  ./deploy.sh --all              # 构建前端+推送所有代码"
            echo ""
            echo -e "${YELLOW}调试和测试:${NC}"
            echo "  ./deploy.sh --dry-run          # 预览模式，不执行实际操作"
            echo "  ./deploy.sh --health-check     # 仅执行健康检查"
            echo "  ./deploy.sh --skip-tests       # 跳过集成验证（快速部署）"
            echo "  ./deploy.sh --force            # 强制部署（跳过确认）"
            echo ""
            echo -e "${YELLOW}兼容模式:${NC}"
            echo "  ./deploy.sh build              # 仅构建前端（不部署）"
            echo ""
            echo -e "${YELLOW}常用示例:${NC}"
            echo -e "${CYAN}  # 修改前端后部署${NC}"
            echo "  ./deploy.sh --frontend"
            echo ""
            echo -e "${CYAN}  # 修改后端后部署${NC}"
            echo "  ./deploy.sh --backend"
            echo ""
            echo -e "${CYAN}  # 快速部署（跳过测试）${NC}"
            echo "  ./deploy.sh --skip-tests"
            echo ""
            echo -e "${CYAN}  # 预览将要执行的操作${NC}"
            echo "  ./deploy.sh --dry-run"
            echo ""
            echo -e "${YELLOW}目标环境:${NC} http://45.149.156.216:3001"
            exit 1
            ;;
    esac
done
